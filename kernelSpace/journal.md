**Ядро операционной системы**

kernel_init производит базовую инициализацию стека и обработчика прерываний

Добавлена возможность в ядре переключать расширения (extensions), находящаяся в include/kernelSpace/config.h на дебаг(debug) #define \_\_DBCN\_\_ или #undef \_\_DBCN\_\_ легаси(legacy) в функции sbi_console_printf

**Работа с консолью (libsbi/)**
Cледующие описанные функции - интерфйес взаимодействия с SBI. Функции *sbi_debug_console_write* - записать в консоль строку указанного размера, *sbi_debug_console_read* - прочитать из консоли указанное кол-во символов, *sbi_debug_console_write_byte* - записать в консоль символ. Также есть устаревшие функции *sbi_console_putchar* - вывести символ в консоль, *sbi_console_getchar* - считать символ с клавиатуры. 
Также добавлена функция *sbi_conosle_printf* - которая асбтрагирует от SBI

**Исключение (arch/riscV/panic.S, .../htrap.c, .../trap.S)**
Добавлен макрос PANIC для отслеживания в какой строчке и файле упала программа
Загружен обработчик исключений в регистр stvec, который вызывает макрос PANIC, сообщая какой тип исключения произошел
TO DO:
- Обработка типичный исключений

**Виртуальная память (arch/riscV/memory/alloc.c,  .../mmu.c)**
В alloc.c происходит выделение физической памяти из пула \_free\_ram\_start под размер страницы 4096 байт
В mmu.c создается таблица виртуальных адресов согласно правилам Sv32 (таблица которая хранит физический адрес, а ключ это виртуальный адрес. Уровень таблицы зависит от SvX. В данном случае таблица состоит из двух уровней), чтобы в дальнейшем передать таблицу в регистр satp, для активации виртуальной памяти. В таблицу записываются все сегменты (которые находятся в линкере main.lds) программы
TO DO:
- Реализовать очистку физического адреса и страничной таблицы
